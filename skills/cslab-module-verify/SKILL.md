...
uv run python .opencode/skills/skills/cslab-module-contract/scripts/audit_module_source.py `
  --family <generic|dynamic-v1> <目标.py>
```

该脚本检查 Python 语法、中文模块/类/方法 docstring、残留 `print`、明显未使用的私有方法
和私有状态；`dynamic-v1` 还检查无依据的 `result`、`DRun` 非空返回和有效步长是否来自
`self.Data.PGV["DT"]`。静态结果是预检证据，不能代替模板、控制器、物理方程和运行测试。
...
1. 从实际工程入口确认控制器确为 `DynamicCalculateControlV1`，不把 V1 规则套到 V3/V4。
2. 普通动态设备没有已核实返回消费者时，`Run`/`DRun` 成功路径不返回业务信息，也不定义
   `result` 或空结果占位。
3. 每个状态推进步从 `self.Data.PGV["DT"]` 读取并校验有效步长；测试故意传入不同的兼容
   `dt`，证明它不会改变推进量。
4. 实时展示通过同名实例属性验证，下游边界通过实际出口节点验证；没有对应消费者的通道
   记为不适用。
5. `Data.PGV["DT"]` 或候选动态状态无效时，确认当前步不推进、旧状态和出口保持一致，
   且不会抛异常、发送错误反馈或构造没有消费者的失败返回。
6. 连续记录多个时间步，检查状态方向、守恒残差、边界工况和一拍时序；至少覆盖干/满
   设备、容差内外、候选状态保护和出口发布回滚。
7. 核对单步推进方法一次调用只推进一个 `Data.PGV["DT"]`，不会由 `Run`、`DRun` 和辅助
   方法重复积分；存在事件分段时，各 RK4 子步时长之和必须等于本次 `DT`。
8. 普通常微分动态模型默认验证经典四阶 RK4 的 `k1/k2/k3/k4` 阶段、状态 shape、单位和
