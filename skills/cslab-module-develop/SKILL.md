...
按以下顺序推进，不得把调研、方案确认、后台变量复核或测试契约推迟到编码之后：

```text
风险分级 → 需求事实与测试契约 → 模板/API与运行控制器 → 开发者资料与联网调研
→ 方案和模型假设选择 → 计算过程、方程与变量说明 → 开发者最终确认
→ 编写.py源码 → 源码审计与本地运行验证 → 交付登记
```

1. 第0至2步适用于所有开发任务：先确定风险、目标 `.py`、实际测试位置、数据和验收
   标准，再核实模板注入、`startFun`、控制器版本和每项输出的真实消费者。
2. A 级完整执行第3至5.1步；B/C 级按风险提供契约或行为不变证据。A 级联网前先询问
   开发者有无可提供资料；涉及候选模型、假设或数值方法时，由开发者明确选择。
3. 修改算法 `.py` 前必须完整展示当前技术方案，并取得“完全理解并同意实施”的明确
   确认。存在新增模块契约变量时，还必须独立完成后台配置和模板详情 API 复核。
4. 第6步只实现已确认方案：加载通用契约和所属族 Skill，使用中文完整注释，按消费者
   实现输出，并删除没有契约、物理、消费者、异常隔离、复用或测试依据的代码。
5. 第7至8步必须证明实际加载和执行目标 `.py`，按真实测试入口验证衡算、边界、错误
   传播和输出消费者，再交付源码、模板登记、验证结果、未覆盖范围与遗留风险。

遇到测试契约不完整、开发者资料询问尚未答复、关键方案/假设未选择、最终技术方案未
确认，或新增后台变量尚未通过详情 API 复核时，停在对应门禁并等待，不得先写算法源码。
下文第0至8步是各阶段的完整执行标准；本总览不得替代或降低任何详细要求。
...
编码前向开发者说明主入口（如 `Run`、`RunDynamic`、`RunOde`、`Calculate`）的完整计算
过程：每一步先算什么、依赖什么、修改哪些状态、向哪个输出通道写值。复杂模块按入口分别说明初始化、
...
涉及数值边界时，再按 `cslab-module-contract` 的
...
| 合法边界 | 类型、形状、单位、上下限及边界等号的物理语义 |
| 容差 | `atol/rtol`、单位、依据和适用关系；不得跨物理量复用无依据常量 |
| 保护动作 | 投影、归零、归一化、备用计算、上一有效值、跳过本步或回滚 |
| 守恒与状态 | 被修正量的守恒去向、旧状态保留范围及输出影响 |
| 验证工况 | 正常值、边界值、容差内外、局部失败和恢复后的预期结果 |

保护表中的动作只用于可恢复数值边界，不抛异常、不调用错误反馈。保护改变边界投影、
回退顺序、守恒去向或计算结果时属于 A 级选择，必须纳入技术方案由开发者确认。

开发者确认计算过程、方程表和适用的数值保护表后，才进入实现。
...
6. 数值方法、初值、步长、容差、收敛条件，以及不报错的数值边界保护、回退和状态保留
   策略；
...
2. 加载 `cslab-module-contract` 落实平台通用契约：注入、`startFun`、消费者驱动的返回与
   输出、feedback、中文源码文档、通用数值边界保护和最小必要实现。
...
结果的稳态模块核对返回形态；没有已核实返回消费者的动态模块核对实例属性、出口节点、
状态序列和错误传播，不得为了测试方便新增 `result`。动态入口必须按实际控制器角色核对
`Run`、`RunDynamic` 或特殊 `RunOde`，不得沿用 V1 的 `DRun`。使用 Python 3.7.6，并确认依赖
编译模块与当前平台匹配：
Windows 使用 `cp37-win_amd64.pyd`，Linux 使用对应 CPython 3.7 `.so`。缺少兼容
二进制时才退化为静态审查。

验证必须覆盖开发者选定方案中的衡算闭合、初始/边界条件、数值保护分支和验收标准。动态或
